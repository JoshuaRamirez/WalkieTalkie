...
- Add discovery and capability checkpoints.
  **Both landed (v0):** `verify_envelope` emits `envelope.verify` plus
  (optionally) `capability.verify`; `CapabilityIssuer.issue` emits
  `capability.issue`; `discovery_record.verify_record` emits
  `discovery.verify`; `admission_coupling.admit` /
  `require_admission` emit `admission.evaluate`. Emission topology
  is documented in
...
- Include decision reason codes and artifact versions.
  **Both landed (v0):** every `AuditEvent` carries a `reason_code` (a
  ...
  (`"envelope/v0"`, `"wt-cap+jwt"`, `"wt-discovery-record/v0"`, or
  `"wt-admission/v0"`).
